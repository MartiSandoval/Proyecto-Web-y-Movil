...
import { IUploadedFile, ITramite } from "../../types/tramite";
import { getTramite, crearCita, registrarArchivo } from "../../services/api";
...
export const SubirArchivos = (): JSX.Element => {
  ...
  useEffect(() => {
    if (tramiteId) getTramite(tramiteId).then(setTramite).catch(() => {});
  }, [tramiteId]);

  const handleFilesAdded = (newFiles: File[]) => {
    const entries: IUploadedFile[] = newFiles.map((f) => ({
      nombre: f.name,
      file: f,
      estado: "subiendo",
      progreso: 0,
  ...
  const handleGuardar = async () => {
    ...
    setSaving(true);
    try {
      const cita = await crearCita(tramiteId, fecha, hora);
      ...
      for (const f of files) {
        ...
        await registrarArchivo(citaId, f.nombre, urlData.publicUrl);
      }
